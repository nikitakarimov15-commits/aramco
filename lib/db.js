...
function resetPool() {
  if (poolPromise) {
    poolPromise.then(p => p.end().catch(() => {})).catch(() => {});
  }
  poolPromise = null;
  schemaReady = null;
}
...
async function getClient() {
  ...
  if (!poolPromise) {
    ...
    pool.on("error", () => {
      // When a pooled connection emits an error, reset so next request gets a fresh pool.
      resetPool();
    });
...
async function query(text, params = []) {
  ...
  try {
    ...
    // On connection errors, reset the pool so the next call retries.
    if (
      e.message &&
      (e.message.includes("terminated") ||
        e.message.includes("ECONNREFUSED") ||
        e.message.includes("timeout") ||
        e.message.includes("Connection ended") ||
        e.code === "ECONNRESET")
    ) {
      resetPool();
    }
...
async function ensureSchema() {
  ...
  schemaReady = (async () => {
    try {
      ...
      // Don't cache a failed schema init — let next request retry.
      schemaReady = null;
      throw e;
    }
  })();
